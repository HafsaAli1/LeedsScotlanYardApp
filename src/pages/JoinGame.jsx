...
function JoinGame() {
  ...
  const handleJoin = async () => {
    ...
    try {
      // Call API to join the game with the provided Game ID and player name
      const response = await joinGame(gameId, playerName);

      // Store important info in localStorage to pass to Lobby page
      localStorage.setItem("playerId", response.playerId); // unique ID for this player
      localStorage.setItem("gameId", gameId);             // the game being joined
      localStorage.setItem("playerName", playerName);     // name entered by player

      // Navigate to Lobby page after successful join
      navigate("/lobby");
    } catch (err) {
      // Log error to console for debugging
      console.error(err);

      // Notify user if joining the game failed
      alert("Failed to join game. Make sure the game code is correct.");
    } finally {
